...
SECRETS="${DATA}/config/secrets.yaml"
if [ ! -f "${SECRETS}" ]; then
  HOST_NAME="$(hostname 2>/dev/null || echo homeassistant.local)"
  cat > "${SECRETS}" <<EOF
# Generated by deploy.sh — no tokens. Edit URLs for this box.
ha_external_url: http://${HOST_NAME}:${PORT}
ha_internal_url: http://${HOST_NAME}:${PORT}
EOF
  echo "Seeded ${SECRETS} (placeholder URLs only; no tokens)"
fi
