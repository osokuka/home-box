...
class TuyaLocalClimate(TuyaLocalEntity, ClimateEntity):
    ...
    async def async_turn_on(self):
        ...
        # Power-only (DP true) keeps the last Tuya mode. This heat pump
        # stores "cold" while off, then immediately powers down again.
        if self._hvac_mode_dps and self._hvac_mode_dps.type is bool:
            _LOGGER.info("%s turning on", self._config.config_id)
            modes = self.hvac_modes
            current = self.hvac_mode
            if current in (None, HVACMode.OFF) and HVACMode.HEAT in modes:
